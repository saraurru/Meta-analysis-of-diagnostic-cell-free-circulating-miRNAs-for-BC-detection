...
# tiff(file = "Forest plot1.tiff", width = 6400, height = 3900, units = "px", res = 400)
# par(mfrow = c(1,2))
...
weights <- 4 * rs / max(rs) ##Do we use weights?
...
# Tiff-figures - skip if no output is desired
 tiff(file = "SROCs111.tiff", width = 3600, height = 1900, units = "px", res = 400)
 par(mfrow=c(1,2))
...
# tiff(file = "subgroups2.tiff", width = 3700, height = 3700, units = "px", res = 400)
# par(mfrow=c(2,2))
...
#----6 miRNA-21-----
# Studies with models that analysed miRNA-21 - all reported models

# Preparation of data
mir21 <- db %>% 
  filter(mi_rna_simplified == 'miR-21') %>%
  select(study, model_id, tp,fp,fn,tn) %>%
  na.omit() 
...
# tiff(file = "miR-211.tiff", width = 3600, height = 1900, units = "px", res = 400)
# par(mfrow=c(1,2))

plot.glmer(fit_mir21, xlim=c(0,1), ylim =c(0,1), pty='s', sroclty = 2, predict = T,
           main = "SROC of miR-21 models")
...
# If tiff was ran, move to miRNA-21 on preferred models to add the second plot
...
# SROC
# tiff(file = "subgroup_pref11.tiff", width = 3700, height = 3700, units = "px", res = 400)
# par(mfrow=c(2,2))
...
#----6 miRNA-21-----
# Studies with models that analysed miRNA-21 - preferred models
...
plot.glmer(fit_mir21_pref, xlim=c(0,1), ylim =c(0,1), pty='s', sroclty = 2, predict = T,
           main = "SROC of miR-21 samples on preferred models")
...
# Tiff-figures - skip if no output is desired
# tiff(file = "Infl1.tiff", width = 4700, height = 2100, units = "px", res = 400)
...
# tiff(file = "pref_rat_5c_12.tiff", width = 4800, height = 1800, units = "px", res = 400)
# par(mfrow = c(1,3))
...
# tiff(file = "pref_rat31.tiff", width = 4800, height = 1800, units = "px", res = 400)
# par(mfrow = c(1,3))

# tiff(file = "pref_rat_3c_21.tiff", width = 4000, height = 2300, units = "px", res = 400)
# par(mfrow = c(1,2))
...
# Tiff-figures - skip if no output is desired
# tiff(file = "Pref_alpha_c1_4.tiff", width = 4200, height = 2200, units = "px", res = 400)
...
# Tiff-figures - skip if no output is desired
# tiff(file = "pb1.tiff", width = 3700, height = 2700, units = "px", res = 400)
